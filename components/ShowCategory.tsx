...
const ShowCategory = ({ products, categories, categoryId }: Props) => {
  const router = useRouter();

  const showProducts = (categoryId: string) => {
    return products.filter((product) => product.category._ref == categoryId);
  };
  const nameCategory = (categoryId: string) => {
    return categories.find((category) => category._id == categoryId)?.title;
  };
  return (
    <>
      <Navbar />
